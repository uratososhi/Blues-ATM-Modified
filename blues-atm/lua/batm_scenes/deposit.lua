...
function scene.Load(ent)
	...
	--Deposit all button
	ent:AddButton(ScrW/2 + 320, ScrH / 2, 90, 90, 
		function() --On pressed
			scene.enteredAmount = LocalPlayer():getDarkRPVar("money")
			if BATM.VerifyNumber(scene.enteredAmount) then
				ent:SetScene("loading")
				BATM.Deposit(ent, scene.enteredAmount or 0)
			end
...
function scene.Draw(ent, ScrW, ScrH)
	...
	--Deposit all button
	draw.RoundedBox(0, ScrW/2 + 320, ScrH / 2, 90, 90, Color(16,223,16))
	draw.SimpleText("All", "batm_deposite_text", ScrW/2 + 365, (ScrH/2) + 45, Color(255,255,255,255), 1, 1) 
